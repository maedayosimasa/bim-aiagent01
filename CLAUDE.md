...
- **(2026-08-13追加)法規チェックを4件追加した(換気/容積率/接道長さ/避難歩行距離)。** ユーザーからの「他にチェックできる内容を追加したい」という依頼を受け、Legal Knowledge Builderの`/concepts`(全38概念)と実際のBIMエンジンの資産を突き合わせて実現性を調査した上で実装した(日影規制・高さ制限・斜線制限・防火関連は、シミュレーション級の幾何計算やArchicadカスタムProperty同期(未実装)が必要なため見送った)。
